      ...
      <div class="money-receipt">
        ...
        <div class="receipt-details" style="display: flex; justify-content: space-between;">
            ...
            <div class="payment-type-section">
                <label>
                    Cash
                    <input type="checkbox" 
                           {{ $invest_Amount->collection->payment_type === 'Cash' ? 'checked' : '' }} />
                </label>
                <hr style="margin: 10px 0; border: 1px solid #000000;">
                
                <p>Cheque/DD/PO No.:
                    {{-- {{ $invest_Amount->payment_type == 'Bank' ? $invest_Amount->cheque_no ?? '' : '' }}</p> --}}
                    {{ $invest_Amount->collection->cheque_no ?? '' }}</p>
                <p>Drawn on: {{ $invest_Amount->collection->bank->name ?? '' }}
                </p>
                <p>Date: {{ $invest_Amount->collection->check_issue_date ?? '' }}</p>
            </div>


            <div style="text-align: left; font-size: 14px; margin-bottom: 10px;">
                <p style="margin-bottom: 25px;">Date:
                    {{ date('d.m.Y', strtotime($invest_Amount->collection->date)) }}</p>
                <p style="margin-bottom: 30px;">MR No.: <strong>{{ $invest_Amount->collection->voucher_no }}</strong></p>
                <p>Total Amount:
                    <span style="border: 1px solid black; padding: 5px 10px; display: inline-block;">
                        <strong>{{ number_format($invest_Amount->collect_amount, 2) }} Tk</strong>
                    </span>
                </p>
        ...
        <div class="details-section-receipt">
            <p>Received with thanks from
                <span class="dotted-line"><b>{{ $invest_Amount->collection->consumer->consumer_name ?? '' }}</b></span>
            </p>

            <p>On account of
                <span
                    class="dotted-line"><b>{{ $invest_Amount->collection->consumer->head->head_name ?? 'No Head Name' }}</b></span>
            </p>

            <p>A sum of amount:
                @php
                    $f = new NumberFormatter('en', NumberFormatter::SPELLOUT);
                    $amount = getBangladeshCurrency($invest_Amount->collect_amount);
