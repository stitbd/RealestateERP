...
@section('content')
    <div class="container mt-2">
        <div class="row justify-content-center">
            <div class="col-md-12">
                @if ($pendingCheques > 0)
                    <h5 class="bg-warning p-3 text-center"> <strong> Warning: There are {{ $pendingCheques }} checks in the
                            next 7 days</strong> </h5>
                @endif
                <div class="card card-info card-outline">
                    ...
                    <div class="card-body p-3">
                        <form action="{{ route('expense-voucher') }}" method="post" enctype="multipart/form-data"
                            id="myForm">
                            @csrf
                            <div class="row">
                                <div class="col-lg-6 ">
                                            ...
                                            <select class="form-control chosen-select" name="company_id" id="company"
                                                ...
                                                @foreach ($company as $v_company)
                                                    <option value="{{ $v_company->id }}"
                                                        @if ($v_company->id == $company_id) selected @endif>
                                        ...
                                        <div class="col-lg-12">
                                            ...
                                            <div class="form-check row">
                                                <input class="form-check-input col-sm-1 " type="radio" name="expense_type"
                                                    id="current" value="current" required onclick="showAdvanceExpense()">
                                                <label class="form-check-label col-sm-2 ml-1" for="current">Current</label>
                                                <input class="form-check-input col-sm-1" type="radio" name="expense_type"
                                                    id="advance" value="advance" required onclick="showAdvanceExpense()">
                                                <label class="form-check-label col-sm-2 ml-1" for="advance">Advance</label>

                                                <input class="form-check-input col-sm-1" type="radio" name="expense_type"
                                                    id="requisition" value="requisition" required
                                                    onclick="showAdvanceExpense()">
                                                <label class="form-check-label col-sm-3 ml-1"
                                                    for="requisition">Requisition</label>
                                            </div>
                                        </div>
                                        ...
                                        <div class="col-lg-12" id="advance-expense">
                                            ...
                                            <select class="form-control chosen-select" name="advance" id="advance-id"
                                                ...
                                                @foreach ($advance_expenses as $expense)
                                                    <option value="{{ $expense->id }}">{{ $expense->details }}</option>
                                                @endforeach
                                            </select>
                                        </div>

                                        <div class="col-lg-12 mb-2 mt-1" id="advance-cheque">
                                            <label for="advance">Advance Cheques</label>
                                            <select class="form-control chosen-select" name="advance_cheque"
                                                id="advance-cheque-id" onchange="advanceChequeAmountShow();">
                                                <option value="">Select One</option>
                                                @foreach ($advance_cheques as $cheque)
                                                    <option value="{{ $cheque->id }}">{{ $cheque->benificiary }}</option>
                                                @endforeach
                                            </select>
                                        </div>

                                        <div class="col-lg-12">
                                            <label for="category">Account Head</label>
                                ...
                                <div class="col-lg-6 ">
                                    ...
                                    <div class="row border m-2 p-2" style="border-color: rgb(9, 155, 212) !important">
                                        ...
                                        <div class="col-md-10 mt-3 bank">
                                            <h6 class="text-center text-bold">Bank Inforamtion</h6>
                                        </div>
                                        <div class="col-md-2 mt-3 bank">
                                        ...
                                        <div class="col-lg-12 bank">
                                            ...
                                            <input type="text" name="bank_amount[]" class="form-control bank_info"
                                                id="bank_amount-1" />
                                        </div>
                                        <div class="col-lg-6 bank">
...
@push('script_js')
    ...
    <script>
        ...
        function checkCash() {
            ...
            if (fund == 2) {
                var remain_amount = parseFloat("{{ $remain_amount }}");
                if (remain_amount == 0) {
                    console.log(remain_amount);
                    alert("You don't have sufficient amount to expense.");
        ...
        //=============================== End of Advance Expense ====================//
        //=============================== Advance Cheque ====================//

        function advanceChequeAmountShow() {
            var advance_cheque_id = document.getElementById('advance-cheque-id').value;
            if (advance_cheque_id) {
                $('.bank').show();
                $('.cheque').show();
                var fundSelect = document.getElementById("fund_id");
                var paymentTypeSelect = document.getElementById("payment_type");
                var selectedFundId = fundSelect.querySelector("option[value='1']");
                var paymentTypeOption = paymentTypeSelect.querySelector("option[value='Bank']");

                selectedFundId.selected = true;
                paymentTypeOption.selected = true;
            }
            var url = "{{ route('advance-cheque-amount') }}";
            $.ajax({
                type: "GET",
                url: url,
                data: {
                ...
                success: function(data) {
                    ...
                    $('#bank_id-1').val(data.bank_id).trigger("chosen:updated");
                    $('#account-1').val(data.bank_id).trigger("chosen:updated");
                    $('#account_holder_name_1').val(data.account_holder);
                    $('#bank_amount-1').val(data.amount);
                    $('#cheque1').val(data.cheque_no);
                    $('#cheque2').val(data.cheque_issue_date);
        ...
        // function expenseBnakInfo(){
        //     var advance_cheque_id = document.getElementById('advance-cheque-id').value;
        //     var url = "{{ route('advance-cheque-bank-info') }}";
        //     $.ajax({
        //         type: "GET",
        //         url: url,
        //         data: {
        ...
        //         },
        //         success: function(data) {
        //             var bank = document.getElementById("bank_id-1");
        //             var option = bank.querySelector('option[value="' + data.bank_id + '"]');
        //             console.log(option);
        //             if (option) {
        ...
        <div class="col-md-12  mt-3 append-` + j + `">
            ...
            <div class="form-group row pb-3">
                ...
                <div class="col-lg-6 bank" id="">
                    <label for="cheque_no">Cheque Number</label>
                    <input type="text" name="cheque_no[]" class="form-control"/>
                </div>
                <div class="col-lg-6 bank mb-3" id="">
                    <label for="cheque_issue_date">Cheque Issue Date</label>
                    <input type="date" name="cheque_issue_date[]" class="form-control"/>
                </div>
            </div>
    ...
    `);
        ...
        function filtersubProject() {
            var company_id = document.getElementById('company').value;
            var url = "{{ route('filter-sub-project') }}";
            $.ajax({
                type: "GET",
                url: url,
                data: {
