            ...
            <div class="col-md-12">
                <div class="card card-info card-outline">
                    ...
                    <div class="card-body p-3">
                        <form action="{{ route('update-land-purchase', $purchase->id) }}" method="post"
                            ...
                            <div class="row">
                                    ...
                                    <select name="project_id" id="project_id" class="form-control" required>
                                        ...
                                        @foreach ($project_data as $item)
                                            <option value="{{ $item->id }}"
                                                @if ($item->id == $purchase->project_id) selected @endif>{{ $item->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
                                ...
                                <div class="row">
                                    ...
                                    @foreach ($purchase_details as $index => $detail)
                                        <input type="hidden" name="details_id[]" id=""
                                            value="{{ $detail->id }}">
                                        <div class="row border m-2 p-2" style="border-color: green !important">
                                            ...
                                            <div class="col-lg-12">
                                                ...
                                                <div class="text-right">
                                                    <button type="button" class="btn btn-sm"
                                                        onclick="add_data_no({{ $index }})"
                                                        style="background-color: rgb(1, 91, 41); color:#fff">
                                                        <i class="fa fa-plus"></i> Add More Donor
                                                    </button>
                                                </div>
                                                <div class="row  align-items-center old_donor">
                                                    @if ($detail->donors)
                                                        @foreach ($detail->donors as $i => $donor)
                                                            <div class="col-md-10 mt-2">
                                                                <input type="hidden"
                                                                    name="land_doner_id[{{ $index }}][{{ $i }}]" value="{{$donor->id}}">
                                                                <input type="text" id="dataQuantity1"
                                                                    class="form-control"
                                                                    name="donor_name[{{ $index }}][{{ $i }}]"
                                                                    placeholder="Write the Donor Name(e.g,XYZ)" required
                                                ...
                                                <!-- Dynamic Donor Data -->
                                                <div id="data_name{{ $index }}" class="mt-3 mb-3">

                                                </div>
                                            </div>

                                            <div class="col-lg-4">
                                                <label for="Fund">Dolil No (দলিল নং) <i
                                                        class="text-danger">*</i></label>
                                                <input name="dolil_no[]" placeholder="Dolil No" type="text" required
                                                    class="form-control" value="{{ $detail->dolil_no }}" />
                                            </div>
                                            <div class="col-lg-4">
                                                <label for="Fund">Rs Dag No (আর এস দাগ নং) <i
                                                        class="text-danger">*</i></label>
                                                <input name="rs_dag_no[]" placeholder="Rs Dag No" type="text" required
                                                    class="form-control" value="{{ $detail->rs_dag_no }}" />
                                            </div>
                                            <div class="col-lg-4">
                                                <label for="Fund">Khatian No (খতিয়ান নং)<i
                                                        class="text-danger">*</i></label>
                                                <input name="khatian_no[]" placeholder="Khatian No" type="text" required
                                                    class="form-control" value="{{ $detail->khatian_no }}" />
                                            </div>


                                            <div class="col-lg-4">
                                                <label for="Fund">Khazna No (খাজনা নং) <i
                                                        class="text-danger">*</i></label>
                                                <input name="khazna_no[]" placeholder="Khazna No" type="text" required
                                                    class="form-control" value="{{ $detail->khazna_no }}" />
                                            </div>
                                            <div class="col-lg-4">
                                                <label for="Fund">Percentage (শতাংশ)<i class="text-danger">*</i></label>
                                                <input oninput="calculateTotal(1)" name="shotangso[]"
                                                    placeholder="Shotangso" type="text" required class="form-control"
                                                    id="shotangso1" value="{{ $detail->shotangso }}" />
                                            </div>
                                            <div class="col-lg-4">
                                                <label for="Fund">Amount Per Percentage (প্রতি শতাংশের দাম) <i
                                                        class="text-danger">*</i></label>
                                                <input name="per_shotangso_rate[]" placeholder="Amount Per (Shotangso)"
                                                    type="text" oninput="calculateTotal(1)" required
                                                    class="form-control" id="per_shotangso_rate1"
                                                    value="{{ $detail->per_shotangso_rate }}" />
                                            </div>


                                            <div class="col-lg-4">
                                                <label for="Fund">Total Amount <i class="text-danger"></i></label>
                                                <input readonly name="total_amount[]" placeholder="Total Amount"
                                                    type="text" required class="form-control" id="total_amount1"
                                                    value="{{ $detail->total_amount }}" />
                                            </div>
                                            <div class="col-lg-4">
                                                <label for="Fund">Middle-man Amount <i
                                                        class="text-danger">*</i></label>
                                                <input name="medium_amount[]" placeholder="Middle-man Amount"
                                                    type="text" required class="form-control"
                                                    value="{{ $detail->medium_amount }}" />
                                            </div>
                                            <div class="col-lg-4 ">
                                                ...
                                                <select name="type[]" id="" class="form-control chosen-select"
                                                    ...
                                                    <option value="1"
                                                        @if ($detail->type == 1) selected @endif>Power</option>
                                                    <option value="2"
                                                        @if ($detail->type == 2) selected @endif>Saf Kabala
                                                    </option>
                                                    <option value="3"
                                                        @if ($detail->type == 3) selected @endif>Namjari</option>
                                                </select>
                                            </div>
                                            ...
                                            <div class="col-lg-12">
                                                ...
                                                <!-- Initial File Input -->
                                                <div class="row g-3 old_file">
                                                    @foreach ($detail->documents as $j => $document)
                                                        <input type="hidden" name="document_id[{{ $index }}][{{ $j }}]" value="{{$document->id}}">
                                                        <div class="col-md-5">
                                                            <input type="text" id=""
                                                                name="land_documents_type_name[{{ $index }}][{{ $j }}]"
                                                                class="form-control" placeholder="Document Name" required
                                                                value="{{ $document->land_documents_type_name }}" />
                                                        </div>
                                                        <div class="col-md-5">
                                                            <input type="file" id="attachFile1"
                                                                name="land_documents[{{ $index }}][{{ $j }}]"
                                                                class="form-control" />
                                                            @if ($document->land_documents)
                                                                <img src="{{ asset('land_purchase/' . $document->land_documents) }}"
                                                ...
                                                <!-- Dynamic File Inputs -->
                                                <div id="attachFileNoDiv{{ $index }}" class="mt-3"></div>
                                            </div>

                                            <div class="col-lg-12">
                                                <label for="remarks">Remarks<i class="text-danger">*</i></label>
                                                <input type="text" name="remarks[]" id="remarks"
                                                    class="form-control" required value="{{ $detail->remarks }}" />
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
...
@push('script_js')
    <script>
       ...
       function add_data_no(sl) {
            let donorCount = $(`.old_donor input[name^="donor_name[${sl}]"]`).length;

            let nextIndex = donorCount;
        ...
        function attachFileNo(sl) {
            let currentCount = $(`.old_file input[name^="land_documents_type_name[${sl}]"]`).length;
            let nextIndex = currentCount;

            $('#attachFileNoDiv' + sl).append(`
            <div class="row g-3 mt-1 align-items-center" id="file_field_${sl}_${nextIndex[sl]}">
                <!-- Document Name Input -->
                <div class="col-md-5">
                    <input
                        type="text"
                        name="land_documents_type_name[${sl}][${nextIndex}]"
                        class="form-control"
                ...
                <div class="col-md-5">
                    <input
                        type="file"
                        name="land_documents[${sl}][${nextIndex}]"
                        class="form-control"
                        required
                    />
                </div>
