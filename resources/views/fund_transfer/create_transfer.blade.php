                    ...
                    <form action="{{route('fund-transfer-store')}}" method="post" enctype="multipart/form-data">
                        ...
                        <div class="row">
                            <div class="col-md-6 border p-3">
                                ...
                                <select name="from_fund_id" id="from_fund" class="form-control" required onchange="showFromBankInfo()">
                                ...
                                <select name="from_bank_id" id="from_bank_id" class="form-control from_bank" onchange="filterAccount()">
                            ...
                            <div class="col-md-6 border p-3">
                                ...
                                <select name="to_fund_id" id="to_fund" class="form-control" required onchange="showToBankInfo()">
                                    <option value="">Select a Fund </option>
                                    @foreach($fund_types as $fund)
                                        <option value="{{$fund->id}}">{{$fund->name}}</option>
                                    @endforeach
                                ...
                                <select name="to_bank_id" id="to_bank" class="form-control to_bank"  onchange="filtertoAccount()">
                                    <option value="">Select a Bank</option>
                                    @foreach($banks as $bank)
                                        <option value="{{$bank->id}}">{{$bank->name}}</option>
                                    @endforeach
                                ...
                                <input type="number" class="form-control" name="amount" placeholder="Enter Transfer Amount" required>

                                <label for=""> Transfer Date </label>
                                <input type="date" name="date" class="form-control" value="{{date('Y-m-d')}}" required>

                                <label for=""> Particulars </label>
                                <input type="text" name="particulars" placeholder="Type Particulars" class="form-control" value="" required>

                                <label for="" class="mt-2"> Attachment </label>
                                <input type="file" name="attachment" placeholder="" class="mt-2" value="" required>
                            </div>
                        </div>
        ...
        $.ajax({
            ...
            success:function(data){
                ...
                $.each(data, function(key, value) {
                    $('#from_account_id').append('<option value="' + value.id + '">' + value.account_no +
                        '</option>');
                });
            },
        });
