...
class HeadToHeadTransferController extends Controller
{
    public function index()
    {
        $data['main_menu']      = "Head_to_head_transfer";
        $data['child_menu']     = "Head_to_head_transfer_log";

        $data['head_transfer']  = HeadToHeadTransfer::where(['company_id' => Session::get('company_id'), 'status' => 1])->get();

        return view('head_to_head_transfer.head_transfer_log', $data);
    }

    public function create()
    {
        $data['main_menu']           = "Head_to_head_transfer";
        $data['child_menu']          = "Head_to_head_transfer_entry";
        ...
        return view('head_to_head_transfer.create', $data);
    }
    ...
    public function store(Request $request)
    {
        ...
        if ($request->from_fund_id) {
            $voucher  = HeadToHeadTransfer::latest()->first();
            ...
            $transfer                          = new HeadToHeadTransfer;
            $transfer->voucher_no              = $voucherNumber;
            $transfer->from_head_id            = $request->from_head_id;
            ...
            $transfer->to_head_id              = $request->to_head_id;
            ...
            if ($request->attachment != null) {
                $newAttachment = 'attachment.' . $request->attachment->extension();
                $request->attachment->move(public_path('/head_attachment'), $newAttachment);
                $transfer->attachment = $newAttachment;
            }
            $transfer->save();
            ...
            $msg = "Head To Head Transfer & Fund Amount Updated.";
            $request->session()->flash('warning', $msg);
        } else {
            $msg = "Head To Head Transfer Declined.Don't Have Sufficient Amount To Fund";
            $request->session()->flash('warning', $msg);
        }
    ...
    {
        $transfer = HeadToHeadTransfer::find($id);
        // dd($transfer);
        ...
        $transfer->status = 2;
        $transfer->update();
        $msg = "Head To Head Transfer Log Removed..";
        return redirect()->back()->with('status', $msg);
    }
}
