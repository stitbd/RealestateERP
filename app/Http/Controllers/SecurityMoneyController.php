...
class SecurityMoneyController extends Controller
{
    public function index(Request $request)
    {
        ...
        $data['child_menu']     = 'security-list';
        $data['projects']       = Project::where(['company_id'=>Session::get('company_id'),'status'=>1])->get();
        $bank_garanties         = SecurityMoney::with('project')->where(['status'=>1]);
        $where = array();
        ...
        $data['security_money'] = $bank_garanties;

        return view('account.security_money.index',$data);
    }
    ...
    public function print(Request $request)
    {
        ...
        //return view('account.security_money.index',$data);
        return view('account.security_money.print',$data);
    }
    ...
    public function pdf(Request $request)
    {
        ...
        $pdf = PDF::loadView('account.security_money.print', $data)->setOptions(['defaultFont' => 'sans-serif']);
        $string = str_replace(' ', '_', Session::get('company_name'));
        return $pdf->download('security-money_'.$string.'.pdf');
    }
    
    public function create()
    {
        $data['main_menu']              = 'accounts';
        $data['child_menu']             = 'create-security';
        $data['projects']       = Project::where(['company_id'=>Session::get('company_id')])->get();
        //dd($data['projects']);
        return view('account.security_money.create',$data);
    }

    public function store(Request $request)
    {
        $model = new SecurityMoney();
        $model->security_date = $request->security_date;
        ...
        $msg = "Security Money Inserted.";
        $request->session()->flash('message',$msg);

        return redirect('security-list')->with('status', $msg);
    }
}
