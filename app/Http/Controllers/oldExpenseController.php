...
class ExpenseController extends Controller
{
    public function index(Request $request)
    {
        try{
            ...
            if(auth()->user()->role == 'SuperAdmin' ||auth()->user()->role == 'Admin'  ){
                $expense_data   = Expense::with('company','fund')->where('status',1)->where('company_id',Session::get('company_id'));
                  if($request->category || $request->head ||$request->search|| $request->fund_id || $request->start_date ||$request->end_date){
                    if($request->category){
                ...
                $expense_data                   = $expense_data->orderByDesc('payment_date');
    ...
    {    try{   
            // if($petty_cash){
                ...
                $fund_log->transection_type     = 'expense';
                ...
                $company_info = Company::where('id',$request->company_id?$request->company_id:session()->get('company_id'))->first();
                // dd($company_info);
                ...
                session([
                    ...
                    'advance_cheque_id'         => $advance_cheque_id,
                    'category'                  => $category,
                    'head'                      => $head,
                ]);
                // dd($bankArray);
                return view('account.expense.print_voucher',compact('model','bankArray','fund_log','category','head','company_info'));

            // }else{
                // $msg="Dont Have Petty Cash Amount.";
...
public function addAttachment(Request $request,$id){
    ...
    public function printDebitVoucher($id){
        try{
            ...
            return view('account.expense.debit_voucher_print',compact('model','receive_by'));
        }catch (\Exception $e) {
            return $e->getMessage();
        }
    }
